...
- R2 is enabled in the owning account. Bucket `utildesk-tool-assets` is Standard/WEUR and production Worker binding `TOOL_ASSETS` is active. Use it for new or changed illustrations; historical objects may remain on the verified Pages/frozen fallback.
