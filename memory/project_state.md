...
- Production deliberately uses separate sitemap contracts for Google and Bing.
- Google / robots / GSC contract:
  - public advertised sitemap: `https://tools.utildesk.de/sitemap.xml`;
  - `site/public/robots.txt` must advertise only this conservative sitemap;
  - this sitemap contains the staged Google-quality surface only: core tools, popular tools, partner/affiliate tools, categories, Ratgeber, and localized equivalents;
  - after the 2026-05-06 fix and rebase, live GSC downloaded this file successfully with `submitted = 322`, `warnings = 0`, `errors = 0`.
- Bing contract:
  - broad sitemap: `https://tools.utildesk.de/sitemap-bing.xml`;
  - generated by `site/scripts/generate_sitemap.mjs` alongside `sitemap.xml`;
  - contains the broader catalogue surface: `2092` live URLs after the 2026-05-06 deploy;
  - do not advertise it in `robots.txt`; submit it explicitly through Bing Webmaster API/UI only when we intentionally want to refresh Bing.
...
- Implementation points:
  ...
  - `scripts/bing_webmaster_api.py` defaults feed submission to `/sitemap-bing.xml`;
  - `docs/04_operations/bing_webmaster_access.md` documents the split.
- Operational guardrail:
  - do not collapse the two files back into one sitemap unless explicitly changing the search strategy;
  - do not "simplify" Google staging by globally noindexing long-tail pages, because that also damages Bing;
  - if Bing is healthy, do not resubmit its feed reflexively during Google-only fixes.
