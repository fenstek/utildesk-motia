...
- The runtime merges compact static sitemap entries from `utildesk-motia.pages.dev` with D1 Ratgeber URLs, keeping the existing robots/GSC/Bing contract while allowing future Ratgeber publication without a static build. Runtime cache keys include each D1 source revision/hash, so editorial updates bypass stale HTML automatically; sitemap cache TTL is five minutes.
