...
- Published all 16 cards through the bounded tool runtime from source commit `5e2a7a0c`: 32 DE/EN entries in D1, 16 content-addressed R2 WebP objects and eight transactional D1 statements. The pre-release D1 export is `C:\Users\sserg\AppData\Local\Temp\utildesk-runtime-backups\2026-07-19\before-hype16.sql`; runtime report is outside Git under `C:\Users\sserg\AppData\Local\Temp\utildesk-runtime-releases\2026-07-19-hype16-attempt2`.
- The release manifest is `docs/04_operations/illustration_batches/2026-07-19-hype16-editorial-illustrations.json`. Production proof covered 32 runtime pages and 16 R2 images: all pages returned `200` with `X-Utildesk-Content-Runtime: tools-v1`, each image returned `200 image/webp`, and the automatic-content banner was absent. Both IndexNow endpoints accepted all 32 canonical URLs with `200`.
- Sheet rows 1777-1792 were transitioned from `NEW` to `DONE` with `editorial_reviewed=true`, DE/EN, illustration and runtime-release notes. The static batch guard's image assertion still expects `/images/tools/...`; runtime correctly rewrites that source path to hashed `/tool-assets/...` R2 URLs, so runtime releases must use the R2-aware live proof rather than treat this static-path mismatch as a content failure.
...
- `aider`: removed the repetitive generic CLI copy and built the card around a reproducible configuration bug, repository map, diff, tests and Git undo/commit controls; preserved five alternatives, ten FAQ and the existing illustration.
- `langchain`: rewrote the card around a duplicate-payment support workflow with LangGraph persistence, human approval and LangSmith tracing; normalized the five existing alternative links and heading in both locales without changing the illustration.
- `crew-ai`: corrected the malformed price metadata and replaced the role-play-heavy copy with a source-controlled market-briefing crew inside a deterministic flow; preserved four alternatives, expanded both locales to eleven FAQ and retained the illustration.
- `perplexity`: replaced repeated cluster boilerplate with a competitor-research story that separates source discovery, primary-source checks and inference; preserved five alternatives, expanded both locales to eight FAQ and retained the illustration.
- `n8n`: rebuilt the card around an invoice workflow with deterministic validation, approval, execution tracing, idempotent retries and an explicit error path; preserved five alternatives, all related-guide links and the existing illustration.
- `zapier`: replaced the generic agent copy with a lead-to-CRM workflow covering validation, deduplication, human approval and retry safety; replaced irrelevant model alternatives with five active automation peers and preserved eleven FAQ plus the existing illustration.
- `microsoft-power-automate`: corrected the malformed official URL and rebuilt the card around a SharePoint purchase approval with timeout, Try/Catch scopes, bounded retry and governed connections; replaced irrelevant model alternatives with five active automation peers and preserved eleven FAQ.
- `openai-api`: corrected URL and pricing metadata and replaced generic copy with a multimodal address-change workflow using Responses, Structured Outputs, function calling, deterministic validation and evals; preserved four active alternatives, eleven FAQ and the existing illustration.
- `hugging-face`: reframed the broad platform description as a German ticket-classification model selection with license review, owned eval data, Space review and an explicit deployment choice; preserved five alternatives and the existing illustration while expanding both locales to eight FAQ.
- `open-webui`: rebuilt the card around a split local/cloud internal knowledge pilot with scoped Knowledge access and root-equivalent Tool permissions; preserved four alternatives and the illustration, expanded both locales to eight FAQ and avoided adding `lastReviewed` to this tier-C card.
- `copy-ai`: replaced generic GTM prose with a multi-role product-launch handoff grounded in Infobase, Brand Voice and format-specific Content Agents; preserved four alternatives and the illustration, expanded both locales to eight FAQ and kept tier-C metadata free of `lastReviewed`.
- `8x8`: reframed the generic XCaaS overview as a Monday queue-spike investigation linking routing, real-time and historical analytics, CRM handoff and recording governance; preserved four communications alternatives, eight FAQ and the existing illustration in both locales.
- `magma`: replaced abstract collaboration copy with a distributed game-art review where illustrator, writer and art director resolve pose, prop and lighting on one moderated canvas; preserved five differentiated alternatives, eight FAQ and the existing illustration in both locales.
- `capcut`: rebuilt the duplicated feature copy around a restaurant's same-day social clip, transcript-based editing, manually corrected captions, platform-safe framing and media-rights review; preserved five video-editor alternatives, eight FAQ and the existing illustration in both locales.
- `groq`: replaced the hardware-comparison copy with a latency-sensitive repair-shop voice agent covering TTFT percentiles, rate limits, tool validation, fallbacks and production measurement; linked five active inference alternatives and preserved eight FAQ plus the existing illustration.
...
- The full Astro build completed with 1,088 pages and the compact sitemap unchanged at 146 URLs. Production release from clean source commit `942c43cc` published all 100 DE/EN runtime projections, verified source hashes and the 50 existing content-addressed WebP assets, and passed 350/350 live HTML/JSON/Markdown/asset checks. Both IndexNow endpoints accepted all 100 changed canonical URLs with `200`; the private D1 backup is `C:\Users\sserg\AppData\Local\Temp\utildesk-runtime-backups\2026-07-31\before-verdicts50.sql` and the release report is `C:\Users\sserg\AppData\Local\Temp\utildesk-runtime-releases\2026-07-31-verdicts50-production`.

## 2026-07-31 - Bing portal inspection and bounded recovery requests

- Authenticated Bing URL Inspection reported `Discovered, but content crawl not performed` / `URL cannot appear on Bing` for the homepage, Ratgeber hub, Methodik, ChatGPT, OpenAI Codex and the Agentic Commerce guide. This is a current index-layer exclusion even where the API retains historical crawl metadata.
