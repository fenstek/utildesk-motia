...
export function buildEntriesUpsertStatement(entries) {
  ...
  const sql = [
    ...
    "  revision = CASE WHEN content_entries.source_hash <> excluded.source_hash OR COALESCE(content_entries.asset_hash, '') <> COALESCE(excluded.asset_hash, '') OR COALESCE(content_entries.robots_policy, '') <> COALESCE(excluded.robots_policy, '') OR COALESCE(content_entries.googlebot_policy, '') <> COALESCE(excluded.googlebot_policy, '') OR content_entries.is_active <> excluded.is_active OR content_entries.route_state <> excluded.route_state OR content_entries.editorial_reviewed <> excluded.editorial_reviewed THEN content_entries.revision + 1 ELSE content_entries.revision END,",
    ...
    "   OR COALESCE(content_entries.robots_policy, '') <> COALESCE(excluded.robots_policy, '')",
    "   OR COALESCE(content_entries.googlebot_policy, '') <> COALESCE(excluded.googlebot_policy, '')",
    "   OR content_entries.is_active <> excluded.is_active",
    "   OR content_entries.route_state <> excluded.route_state",
    "   OR content_entries.editorial_reviewed <> excluded.editorial_reviewed",
