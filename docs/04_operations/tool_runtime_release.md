...
- `TOOL_ASSETS` is optional in code but active in production. The binding points
  to the Standard/WEUR bucket `utildesk-tool-assets`; routine releases with new
  or changed illustrations must use `--asset-bucket utildesk-tool-assets`.
  `--allow-pages-fallback-assets` remains a recovery-only path for unchanged
  historical bytes and must never silently replace a hash.
